...
$.getJSON("data/pdb_year_municipal_selected_var.geojson", blkgrp_data => {
    ...
    blkgrp_lowmod2015_muncipal = L.geoJson(blkgrp_data, {
        style: style,
        onEachFeature: function(feature, layer) {
            layer.on('click', function() {
                ...
                let prop_value = property[details_indicators];
                let thr_indicatorArry = threshold_arr(details_indicators);
                let num_int = threshold_arr(details_indicators).length + 1;
                let which_interval = thr_indicatorArry.findIndex((x) => prop_value < x);

                if (which_interval == -1) {
                    left_margin_value = 40 * (num_int - 1);
                    left_margin_value += 40 * (prop_value - thr_indicatorArry[num_int - 2]) / (thr_indicatorArry[num_int - 2]);
                } else if (which_interval == 0) {
                    //left_margin_value = 0;
                    left_margin_value = 40 * (thr_indicatorArry[which_interval] - prop_value) / (thr_indicatorArry[which_interval]);
                } else {
                    left_margin_value = 40 * which_interval;
                    left_margin_value += 40 * (prop_value - thr_indicatorArry[which_interval - 1]) / (thr_indicatorArry[which_interval] - thr_indicatorArry[which_interval - 1]);
                }

                $("#line-bar").css("margin-left", `${Math.round(left_margin_value)}px`);
